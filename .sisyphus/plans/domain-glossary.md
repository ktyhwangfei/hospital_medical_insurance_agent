...
> **核心目标**: 基于 DDD 思想，为项目创建 `docs/domain-glossary.md` 全局领域知识库，并确立 AI 编程工作流契约。
>
> **交付物**: 一份包含完整限界上下文划分、核心概念字典、DDD 战术分类、业务规则和 AI 编程契约的领域知识库文档。
>
> **预估工时**: 短（约 30 分钟，已预研完成）
> **执行方式**: 单任务，无波次需求
...
用户要求：
1. 在 `docs/domain-glossary.md` 创建**核心概念字典**，包含限界上下文划分、中英文对照、DDD 战术分类、业务规则和生命周期
2. 确立 **AI 编程工作流契约**（先查后写 + 同步更新）

### 预研成果

已完成全量代码分析（domain/adapters/runtime/knowledge_extension/security/model_service/shared 等全部模块），识别出 12 个限界上下文：

| # | 上下文 | 核心文件 | 模型数 |
|---|-------|---------|-------|
| 1 | Patient | `src/domain/patient/models.py` | 1 |
| 2 | Insurance | `src/domain/insurance/models.py` | 1 |
| 3 | OrderFee | `src/domain/order_fee/models.py` | 5 |
| 4 | AuditRisk | `src/domain/audit_risk/models.py` | 4 |
| 5 | DrgDip | `src/domain/drg_dip/models.py` | 4 |
| 6 | MedicalRecord | `src/domain/medical_record/models.py` | 4 |
| 7 | Appeal | `src/domain/appeal/models.py` | 4 |
| 8 | TaskClosure | `src/domain/task/models.py` | 1 (+ RuntimeTask) |
| 9 | SkillTool | `src/domain/skill/models.py` + mcp_registry | 6+ |
| 10 | Knowledge | `src/knowledge_extension/` | 多个实体 |
| 11 | Security | `src/security/` | 多个服务 |
| 12 | ModelService | `src/model_service/` | 5 |
| — | Shared/Common | `src/shared/` + `src/domain/common/` | 多个 |

### 核心发现

- 跨域复合键 `(patient_id, encounter_id)` 是系统级约定
- `Citation` 在 domain 和 knowledge 各有一份，需标记
- `domain/tool/` 完全空目录（无 `__init__.py`）
- `domain/skill/ToolOwner` 与 `domain/common/roles.Role` 部分重复
...
### 交付物
- `docs/domain-glossary.md` — 全局领域知识库文档（~500 行）

### 定义完成
- [ ] 文档写入 `docs/domain-glossary.md`
- [ ] 包含全部 12+ 个限界上下文的完整字典
- [ ] 所有领域概念的 DDD 战术分类标注正确
- [ ] 附录 A 术语索引（按字母排序）完整
- [ ] 附录 B DDD 模式速查表完整
- [ ] AI 编程工作流契约 5 条全部写入
...
- [x] 1. 写入领域知识库文档到 `docs/domain-glossary.md`

  **What to do**:
  将以下完整内容写入 `docs/domain-glossary.md`。文件中包含：

  ├── 1. 限界上下文总览（12 个上下文的职责、依赖、防腐层、跨域复合键）
  ├── 2. 患者上下文（Patient）— Entity + HisPort
  ├── 3. 医保上下文（Insurance）— Entity + InsuranceInterfacePort
  ├── 4. 医嘱费用上下文（Order & Fee）— Aggregate Root + Value Objects
  ├── 5. 审核风险上下文（Audit & Risk）— Aggregate Root + RiskFlag
  ├── 6. DRG/DIP 上下文— 全部 Value Objects
  ├── 7. 病案上下文（Medical Record）— Aggregate Root
  ├── 8. 申诉上下文（Appeal）— Aggregate Root + Entities
  ├── 9. 任务闭环上下文（Task & Closure）— Entity (可变)
  ├── 10. 技能工具上下文（Skill & Tool）— Pydantic models + MCP
  ├── 11. 知识上下文（Knowledge）— 多个实体和服务
  ├── 12. 安全上下文（Security）— 横切 Domain Services
  ├── 13. 模型服务上下文（Model Service）— Gateway + DTOs
  ├── 14. 共享通用层（Shared / Common）— 跨上下文 DTOs
  ├── 15. AI 编程工作流契约（5 条）
  ├── 附录 A：术语索引（~60 条英文→中文映射）
  └── 附录 B：DDD 模式速查表

  完整文档内容见本计划文件末尾的 **"领域知识库完整内容"** 章节。

  **Must NOT do**:
  - 不要修改任何源代码文件（.py, .ts, .js 等）
  - 不要修改 `.sisyphus/` 下的任何文件
  - 不要修改已存在的 `docs/` 下的其他文件

  **Recommended Agent Profile**:
  - **Category**: `writing` — 文档创作任务
  - **Skills**: `[]` — 无需特殊技能

  **Acceptance Criteria**:
  - [ ] `docs/domain-glossary.md` 文件存在，内容与计划中定义的完整内容一致
  - [ ] 包含 12+ 限界上下文
  - [ ] 附录 A 术语索引完整
  - [ ] 附录 B DDD 模式速查表完整
  - [ ] 5 条 AI 编程工作流契约全部写入
  ...
  **QA Scenarios**:
  ```
  Scenario: 验证文档已写入指定路径
    Tool: Bash
    Preconditions: 无
    Steps:
      1. Test-Path -LiteralPath "D:\project\hospital_medical_insurance_agent\docs\domain-glossary.md"
      2. (Get-Item "D:\project\hospital_medical_insurance_agent\docs\domain-glossary.md").Length -gt 10000
    Expected Result: 文件存在且大小超过 10KB
    Evidence: .sisyphus/evidence/domain-glossary-exists.txt

  Scenario: 验证文档关键章节完整性
    Tool: Bash
    Preconditions: 文档已存在
    Steps:
      1. Select-String -Path "D:\project\hospital_medical_insurance_agent\docs\domain-glossary.md" -Pattern "^## "  # 统计二级标题数量
      2. Select-String -Path "D:\project\hospital_medical_insurance_agent\docs\domain-glossary.md" -Pattern "Aggregate Root|Entity|Value Object|Domain Service"
    Expected Result: 二级标题 >= 15 个，DDD 分类关键词全部出现
    Evidence: .sisyphus/evidence/domain-glossary-sections.txt
  ```

  **Commit**: YES
  - Message: `docs: 创建全局领域知识库 docs/domain-glossary.md，基于 DDD 的 12 个限界上下文划分`
  - Files: `docs/domain-glossary.md`

---

## 领域知识库完整内容

> 此部分为 Task 1 需要写入 `docs/domain-glossary.md` 的完整文档内容。
> 执行者请将此整块内容写入目标文件。
...
**规则**：生成任何后端代码（特别是 Domain 层代码）前，必须先读取 `docs/domain-glossary.md`。
...
- [x] F1. **Plan Compliance Audit** — 验证 `docs/domain-glossary.md` 存在且有足够内容
```
```

## 提交策略

- **Task 1**: `docs: 创建全局领域知识库 docs/domain-glossary.md，基于 DDD 的 12 个限界上下文划分`
...
### 验证命令
```bash
Test-Path "D:\project\hospital_medical_insurance_agent\docs\domain-glossary.md"
Select-String -Path "D:\project\hospital_medical_insurance_agent\docs\domain-glossary.md" -Pattern "限界上下文|Aggregate Root|Entity|Value Object|Domain Service|AI 编程工作流契约"
```

### 最终检查清单
- [ ] `docs/domain-glossary.md` 文件已创建
- [ ] 包含 12 个限界上下文定义
- [ ] 全部领域概念已标注 DDD 战术分类
- [ ] 附录 A 术语索引完整
- [ ] 附录 B DDD 模式速查表完整
