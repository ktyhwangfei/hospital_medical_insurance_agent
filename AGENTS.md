...
- 新增后端路由后页面报「暂不可用」类兜底文案，先 `curl` 直连后端端口分辨 404 与 503 再排障方向。运行中的 uvicorn 是单进程无 `--reload`（start-servers.ps1 [2] 段），新路由必须重启进程才生效；404 也会触发前端失败兜底文案（如「Milvus 连接失败」），极易误导为数据源故障（knowledge-map 页实测）。
- D 盘主克隆（`D:\project\hospital_medical_insurance_agent`）不在 orca ws.ps1 管辖内：`ws.ps1` 只扫自身所在目录下含 `src/` 的工作区，`..\ws.ps1` 报文件不存在。该克隆直接用仓库内 `stop-servers.ps1` + `start-servers.ps1`（复用 `.server-ports.json` 持久化端口，不漂移）。
...
- Milvus 动态字段按行存在：规则没有该维度时 `query` 返回的 dict 里**整个键缺失**（不是空串），JSON 序列化后前端拿到 `undefined`。前端展示代码直接 `value.trim()` 会 TypeError 崩组件（knowledge-map 展开报错实测：418 条规则 25 条无 payment_ratio、380 条无 personal_payment_ratio）；且测试夹具填满全部字段会掩盖此类崩溃，夹具必须包含 delete 键的稀疏用例。DTO 中这类字段应声明可选。
