...
import type { AgentResponse, ChatRequest, Citation, SseEvent } from '@/lib/types'
...
function mockCandidates(message: string, response?: AgentResponse): IntentCandidate[] {
  const primary = inferIntentLabel(message, response)
  const base: IntentCandidate[] = [
...
export default function SettlementChat({ currentRole, prefilledMessage, onPrefillConsumed }: { currentRole: string; prefilledMessage?: string; onPrefillConsumed?: () => void }) {
  ...
  const initialTrace = buildGuideTrace('为什么这个患者结算失败', currentRole, {
    scenario: 'settlement_exception_guidance',
    status: 'completed',
    result: {},
    citations: [
      { source_type: 'intent_knowledge', source_id: 'intent-settlement', summary: '结算失败、错误码、收费窗口异常归入医保结算异常导办' },
      { source_type: 'runtime_context', source_id: 'P001:E001', summary: '当前页面携带患者与就诊上下文' },
    ],
    tasks: [],
    missing_fields: [],
    uncertainties: [],
    blocked_actions: [],
    audit: { workflow_id: 'wf-preview-intent' },
  })
  ...
  const [guideTrace, setGuideTrace] = useState<GuideTrace>(initialTrace)
  ...
  const handleSend = async (text?: string) => {
    ...
    setGuideTrace({
      ...buildGuideTrace(messageText, currentRole),
      stages: [
        { id: 'rewrite', label: 'Query Rewrite', description: '正在结合角色、患者和页面上下文补全问题', status: 'running' },
        { id: 'retrieval', label: 'RAG 候选召回', description: '等待召回意图定义和场景边界', status: 'pending' },
        { id: 'intent', label: 'LLM 意图判别', description: '等待模型结构化判别', status: 'pending' },
        { id: 'guardrail', label: '安全与路由校验', description: '等待校验高风险动作和路由状态', status: 'pending' },
      ],
    })
    setInput('')
    setIsLoading(true)
    ...
    try {
      await sendChatStream(request, (event: SseEvent) => {
        ...
        if (event.event === 'final') {
          ...
          setGuideTrace(buildGuideTrace(messageText, currentRole, agentResponse))
        ...
        if (event.event === 'error') {
          ...
          setGuideTrace({
            ...buildGuideTrace(messageText, currentRole),
            routeStatus: '请求失败，等待重试',
            stages: [
              { id: 'rewrite', label: 'Query Rewrite', description: '已保留原始问题', status: 'done' },
              { id: 'retrieval', label: 'RAG 候选召回', description: '请求失败，未能完成召回', status: 'blocked' },
              { id: 'intent', label: 'LLM 意图判别', description: '请求失败，未能完成判别', status: 'blocked' },
              { id: 'guardrail', label: '安全与路由校验', description: '未进入业务路由', status: 'blocked' },
            ],
          })
      ...
      setGuideTrace({
        ...buildGuideTrace(messageText, currentRole),
        routeStatus: '网络异常，未进入导办流程',
        stages: [
          { id: 'rewrite', label: 'Query Rewrite', description: '已保留原始问题', status: 'done' },
          { id: 'retrieval', label: 'RAG 候选召回', description: '后端不可用，未能召回候选', status: 'blocked' },
          { id: 'intent', label: 'LLM 意图判别', description: '后端不可用，未能判别', status: 'blocked' },
          { id: 'guardrail', label: '安全与路由校验', description: '未进入业务路由', status: 'blocked' },
        ],
      })
  ...
  const confidenceText = `${clampPercent(guideTrace.confidence)}%`
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[calc(100vh-200px)]">
      ...
      <Card className="lg:col-span-1 flex flex-col border-slate-200/70 shadow-sm">
        ...
        <CardContent className="flex-1 flex flex-col space-y-1.5 px-4 pb-5">
          {quickQuestions.map((q, i) => (
            <Button
              key={q}
              variant="ghost"
              className="w-full justify-start text-left h-auto py-2.5 px-3.5 whitespace-normal rounded-xl border border-transparent hover:border-blue-100 hover:bg-blue-50/70 hover:text-blue-700 transition-all duration-200 text-slate-600 group"
      ...
      <Card className="lg:col-span-3 flex flex-col border-slate-200/70 overflow-hidden shadow-sm bg-gradient-to-b from-slate-950 to-slate-900 text-white">
        <CardHeader className="border-b border-white/[0.06] py-4 px-5 bg-gradient-to-r from-slate-900 via-slate-900 to-slate-800/90">
        ...
        <CardContent className="flex-1 grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_340px] min-h-0 p-0">
            ...
            <ScrollArea className="flex-1 px-6 pt-5 pb-3">
              ...
              <div className="mb-5 grid grid-cols-3 gap-3">
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-3 shadow-sm">
                  ...
                  <div className="mt-1 truncate text-sm font-semibold text-white/90">{guideTrace.intentLabel}</div>
                </div>
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-3 shadow-sm">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-white/[0.06]">
                      <div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-400" style={{ width: confidenceText }} />
                    </div>
                    <span className="text-sm font-semibold text-white/90">{confidenceText}</span>
                  </div>
                </div>
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-3 shadow-sm">
                  ...
                  <div className="mt-1 truncate text-sm font-semibold text-white/90">{guideTrace.routeStatus}</div>
                </div>
              </div>
              ...
              <div className="space-y-5">
                {messages.map((msg, idx) => (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`px-5 py-3 text-sm leading-relaxed tracking-normal whitespace-pre-wrap ${
                          msg.role === 'user'
              ...
              {isStreaming && streamingContent && (
                <div className="flex items-end gap-3 flex-row">
                  ...
                  <div className="max-w-[80%]">
                    <div className="bg-white/95 text-slate-800 border border-white/20 rounded-2xl rounded-tl-md shadow-sm px-5 py-3">
              ...
              {isLoading && !isStreaming && (
                <div className="flex items-end gap-3 flex-row">
                  ...
                  <div className="border border-white/20 rounded-2xl rounded-tl-md shadow-sm px-5 py-3.5 bg-white/95">
          ...
          <div className="p-4 border-t border-white/[0.06] bg-slate-900/80 backdrop-blur-sm">
            <div className="flex gap-2.5 items-end">
              <Input
                ...
                className="flex-1 h-auto py-3 px-4 text-sm bg-white/[0.06] text-white/90 placeholder:text-slate-500 border-white/[0.08] rounded-xl focus:bg-white/[0.1] focus:ring-2 focus:ring-blue-400/15 focus:border-blue-400/40 transition-all duration-200"
                disabled={isLoading}
              />
              <Button
                onClick={() => handleSend()}
                ...
                size="icon"
                className="h-[46px] w-[46px] rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 text-white hover:from-blue-400 hover:to-blue-500 disabled:from-slate-700 disabled:to-slate-700 disabled:text-slate-500 disabled:cursor-not-allowed transition-all duration-200 shadow-lg shadow-blue-900/30 active:scale-95"
              >
                {isLoading ? (
          ...
          <aside className="hidden xl:flex min-h-0 flex-col bg-slate-900/60 backdrop-blur-sm">
            <div className="border-b border-white/[0.06] p-4">
              ...
              <p className="mt-1 text-xs leading-relaxed text-slate-500">展示问题改写、候选召回、LLM 判别、路由校验与引用依据。</p>
            </div>
              ...
              <div className="space-y-4">
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="mb-2 flex items-center gap-2 text-xs font-semibold text-slate-400">
                    <RotateCcw className="w-3.5 h-3.5 text-cyan-400/70" /> Query Rewrite
                  </div>
                  <div className="rounded-lg bg-slate-950/60 p-3 text-[11px] leading-relaxed text-slate-500">
                    <div className="mb-1.5 text-slate-600">原始问题</div>
                    <div className="text-slate-200">{guideTrace.originalQuery}</div>
                    <div className="mb-1.5 mt-3 text-slate-600">改写后</div>
                    <div className="text-cyan-200/90">{guideTrace.rewrittenQuery}</div>
                  </div>
                </div>

                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="mb-3 flex items-center gap-2 text-xs font-semibold text-slate-400">
                    <GitBranch className="w-3.5 h-3.5 text-violet-400/70" /> TopN 候选意图
                  </div>
                  <div className="space-y-3">
                    {guideTrace.candidates.map((candidate) => (
                      <div key={candidate.id} className="space-y-1.5">
                        ...
                        <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.06]">
                          <div className="h-full rounded-full bg-gradient-to-r from-cyan-400/80 to-violet-400/80" style={{ width: `${clampPercent(candidate.score)}%` }} />
                        </div>
                        <Badge variant="outline" className="border-white/[0.06] bg-white/[0.04] px-1.5 py-0 text-[10px] text-slate-400">{candidate.status}</Badge>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="mb-3 flex items-center gap-2 text-xs font-semibold text-slate-400">
                    <Database className="w-3.5 h-3.5 text-emerald-400/70" /> RAG 依据召回
                  </div>
                  <div className="space-y-2.5">
                    {guideTrace.evidences.map((item) => (
                      <div key={`${item.source}-${item.title}`} className="rounded-lg border border-white/[0.06] bg-slate-950/50 p-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="text-xs font-semibold text-white/80">{item.title}</div>
                          <span className="rounded-full bg-emerald-400/10 px-2 py-0.5 text-[10px] text-emerald-300">{item.score}</span>
                        </div>
                ...
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="mb-3 flex items-center gap-2 text-xs font-semibold text-slate-400">
                    <ShieldCheck className="w-3.5 h-3.5 text-amber-400/70" /> 执行链路
                  </div>
                  <div className="space-y-2.5">
                    {guideTrace.stages.map((stage) => (
                ...
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="mb-2 flex items-center gap-2 text-xs font-semibold text-slate-400">
                    <FileText className="w-3.5 h-3.5 text-blue-400/70" /> 审计追踪
                  </div>
                  <div className="rounded-lg bg-slate-950/60 p-3 text-[11px] leading-relaxed text-slate-500">
                    <div>workflow_id：<span className="text-slate-200">{guideTrace.auditId}</span></div>
                    <div>citations：<span className="text-slate-200">{guideTrace.citations.length}</span></div>
