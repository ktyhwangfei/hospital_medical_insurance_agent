...
以下批次归并了 Task 2 实际执行的只读查询。它只返回时间、主体哈希、状态枚举和聚合计数，不返回任何交易标识或明细键值；其中 `recent_30d` 仅是服务器时钟观察标签，不代表业务最近 30 天口径已获确认。
...
|---|---|---|---|
| o_Trade 行数 | 592 | 0（非可靠最近 30 天证据） | 观察窗口 **BLOCKED**，不据此判断近期无交易 |
| T_SetTid NULL / 去重数 | 11 / 257 | 0 / 0（观察值） | 全量证据不满足内部结算锚点的一对一要求 |
| T_SetTid 重复组数 / 涉及行数 | 2 / 326 | 0 / 0（观察值） | 基于全量证据 **BLOCKED**：停止 `settlement_id → 单笔交易` 假设，P1 阻断 |
| T_TradeNo NULL / 去重数 | 0 / 592 | 0 / 0（观察值） | 仅基于全量 592 行及物理主键，冻结为交易业务键 |
| T_TradeNo 重复组数 / 涉及行数 | 0 / 0 | 0 / 0（观察值） | 全量结论冻结，观察窗口不参与冻结 |
| o_FeeItem 行数 | 2,139 | 0（按父交易日期得到的观察值） | 不据此判断近期无明细 |
| 复合键三个分量 NULL / 任一 NULL 行 | 0 / 0 / 0；任一 NULL 0 | 均为 0（观察值） | 全量证据通过 |
| 复合键重复组数 / 涉及行数 | 0 / 0 | 0 / 0（观察值） | 仅基于全量证据冻结 `(T_TradeNo, ItemId, ItemNo)` 为费用明细幂等键 |
| o_FeeItem → o_Trade 孤儿明细数 | 0 | 未验证（时间口径阻断） | 全量 T_TradeNo 主从关系无歧义；本范围支持一笔交易对应多条费用明细 |
...
| T2-B02 | T_TradeDate 为无时区 datetime，尚未取得其业务时区与时钟语义；以服务器 SYSDATETIME() 形成的窗口只观察到交易/关联明细 0 | 不能把该观察值作为可靠最近 30 天证据；不影响全量键、重复和主从关系结论 | 由数据负责人确认 T_TradeDate 的业务时区/时钟语义，并按确认后的时间口径重新执行参数化窗口查询 |
...
Task 1 已形成两张候选表的发现证据草稿、数据库版本/脱敏权限位、236 个字段物理类型以及键/索引/FK 初步元数据观察。Task 2 已通过批准的注册表入口完成全量键、重复、孤儿和状态组合聚合；服务器时钟窗口查询仅保留为观察值，业务最近 30 天口径为 BLOCKED。仅执行只读语句，没有修改生产代码或源库。该草稿不代表正式审核完成。
