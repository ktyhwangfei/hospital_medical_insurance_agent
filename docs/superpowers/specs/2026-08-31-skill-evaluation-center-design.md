...
- 真实 Policy QA 只能执行当前工作区已物化的 Skill 制品，因此 Benchmark 启动前必须校验候选版本的制品哈希与当前运行时一致；不一致时阻断，禁止把版本元数据当成实际执行版本。
- 独立候选/基线隔离执行、按任务指纹比较和历史趋势仍需版本制品存储与隔离运行器支撑。首版 API 对 `baseline_version_id` 明确返回 `baseline_isolation_unavailable`，页面只展示待接入状态，不生成伪基线数据。
- 后续开放基线选择前，必须先实现可按 `version_id` 装载完整 Skill 制品的隔离执行器，并补齐候选/基线同环境运行及 `max_new_failures` 门禁。
