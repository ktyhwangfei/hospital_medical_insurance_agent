...
|---|---|---|---|---|
| P0 数据契约 | blocked（评审已执行，准入未通过） | `2026-08-27-outpatient-p0-data-contract.md` | G01/G04 技术证据已闭合；v3 查询模型和主要值域已自主核验 | 完成剩余业务签认、票据、CDC、只读权限、隐私和正式签字后重跑 P0 门禁 |
| P1 近实时数据底座 | blocked | P0 准入前不得进入规划；仅补真正外部证据并重新执行 P0 门禁 | P0 准入通过，且可靠增量游标已冻结 | PostgreSQL 单一路径刷新 P95 不超过 5 分钟，批次原子发布，运营语义版本绑定可追溯数据批次 |
...
P0 评审工作已执行完毕，但准入门禁未通过。G01 内部锚点和 G04 `o_FeeItem` 技术选型已闭合；`mzjyxx` v3 已发布且 `queryable=true`，主要值域和 88 个门诊核验依赖已从语义层自主核验。当前按 [`P0 收口签认包`](../../reviews/2026-08-28-outpatient-p0-closure-signoff-package.md) 处理仍需业务决定或外部系统动作的部分并重新执行 P0 门禁；不得进入 P1 规划或生产代码开发，也不得重复建设查询模型或向医院索要系统内已有资料。
