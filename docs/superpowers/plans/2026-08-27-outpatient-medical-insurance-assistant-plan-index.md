...
|---|---|---|---|---|
| P0 数据契约 | blocked（评审已执行，准入未通过） | `2026-08-27-outpatient-p0-data-contract.md` | 已完成 Task 1–7 证据评审；G01 技术证据已纠偏闭合 | 补齐评审记录 G02–G13 的证据并获医保办/数据负责人实际签认后，重新执行 P0 门禁 |
| P1 近实时数据底座 | blocked | P0 准入前不得进入规划；仅补证据并重新执行 P0 门禁 | P0 准入通过，且明细源和可靠增量游标已冻结 | PostgreSQL 单一路径刷新 P95 不超过 5 分钟，批次原子发布，`mzjyxx.queryable=true` |
...
P0 评审工作已执行完毕，但准入门禁未通过。G01 内部锚点已纠偏为 `T_TradeNo` 并完成技术补证；当前只允许按评审记录 G02–G13 补证据、完成真实签认并重新执行 P0 门禁；不得进入 P1 规划或生产代码开发，不发布费用明细源，不创建可查询语义版本。
