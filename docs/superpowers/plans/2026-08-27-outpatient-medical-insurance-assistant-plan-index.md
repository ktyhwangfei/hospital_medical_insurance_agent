...
P0 评审工作已执行完毕，但准入门禁未通过。G01 内部锚点和 G04 `o_FeeItem` 技术选型已闭合；`mzjyxx` v3 已发布且 `queryable=true`，主要值域和 88 个门诊核验依赖已从语义层自主核验。当前只处理评审记录中仍需业务决定或外部系统动作的部分并重新执行 P0 门禁；不得进入 P1 规划或生产代码开发，也不得重复建设查询模型或向医院索要系统内已有资料。
