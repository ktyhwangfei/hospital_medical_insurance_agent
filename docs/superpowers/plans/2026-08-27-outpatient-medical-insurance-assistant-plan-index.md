...
| P1 近实时数据底座 | planned | `2026-08-28-outpatient-p1-near-real-time-data-foundation.md` | 已满足：P0 设计确认；CDC/只读账号作为实施任务而非设计问卷 | PostgreSQL 单一路径刷新 P95 不超过 5 分钟，批次原子发布，运营语义版本绑定可追溯数据批次 |
...
P0 事实探查和设计确认已完成，D01–D12 全部采用。P1 最小实施计划已形成，当前执行点进入 **P1 Task 1：落地 Issue20 查询模型契约的最小子集**；外部 CDC、只读账号和扫码/SSO 作为实施任务，不再重复字段发现或方案问卷。
