...
def _db_ready() -> bool:
    ...
    try:
        ...
        conn = pyodbc.connect(
            ...
            "UID=sa;PWD=Figo@1472583690", timeout=2)
        conn.close()
        return True
    except Exception:
        return False
...
def test_search_database_via_semantic_source():
    ...
    conn = pyodbc.connect(
        "DRIVER={SQL Server};SERVER=127.0.0.1,1433;DATABASE=bjybdb;"
        "UID=sa;PWD=Figo@1472583690", timeout=3)
    cur = conn.cursor()
...
import pyodbc
conn = pyodbc.connect('DRIVER={SQL Server};SERVER=127.0.0.1,1433;DATABASE=bjybdb;UID=sa;PWD=Figo@1472583690',timeout=3)
