...
## 14. Phase 4 性能基线与运行运营决策（2026-09-07）

Phase 4 首个最小可验证单元为已发布 View 的真实 PostgreSQL 查询性能基线，
不提前引入物化表、调度器、缓存或异步运行层。基准复用
`PostgresFlowViewReader.read()`，顺序热连接 30 次、10 个独立连接并发 50 次，
只测查询热路径，不计首次连接建立时间；只读接口 P95 阈值沿用 300ms。

当前活库结果：顺序 P50/P95 为 1.82ms/2.91ms，并发 P50/P95 为
2.81ms/4.30ms，均通过阈值。当前 View 仅有 1 行快照，结果只作为小规模基线，
不能外推生产大数据量。

决策：当前不增加物化、调度或缓存复杂度。接入代表性生产规模数据后，若顺序或
目标并发 P95 超过 300ms，或出现明确数据库资源瓶颈，再评估物化/调度/缓存，
并同时验证刷新延迟、发布版本一致性、缓存失效与回滚行为。

详细证据：`docs/reviews/2026-09-07-issue65-phase4-performance.md`。
