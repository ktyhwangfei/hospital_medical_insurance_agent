...
│   ├── config/
│   │   └── semantic_mapping.yaml       # 政策语义映射配置
│   └── test_*.py                       # 各模块测试
├── crawl/                              # 政策爬虫
├── policy_extract/                     # 政策提取
├── policy_fact/                        # 政策事实
...
- SQL Server 业务查询统一由 `src/semantic_layer/query_planner.py` 基于已发布模型编译

## 注意事项

- `policy_retrieval/` 下仅 `milvus_retriever.py`、`reranker.py`、`query_understanding.py`、`semantic_mapping.py`、`explanation_planner.py` 为核心检索管线组件，其余为辅助/测试模块
