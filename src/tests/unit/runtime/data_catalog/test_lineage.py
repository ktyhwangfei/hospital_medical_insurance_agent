...
from src.runtime.data_catalog.lineage import _derive_edges, derive_lineage
...
def _seeded_store() -> InMemoryDataCatalogStorage:
...
class TestDeriveEdges:
    def test_full_chain_edges(self) -> None:
        ...
        store = _seeded_store()
        edges = _derive_edges(store.list_assets(limit=100))
        triples = {(e.upstream, e.downstream, e.relation) for e in edges}
    ...
    def test_consumer_matched_by_domain_code(self) -> None:
        ...
        triples = {
            (e.upstream, e.downstream, e.relation)
            for e in _derive_edges(store.list_assets(limit=100))
        }
