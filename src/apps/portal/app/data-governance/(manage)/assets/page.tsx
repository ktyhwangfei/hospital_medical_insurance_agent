...
import {
  ...
  Table2, Brain, Sigma, MousePointerClick, Network, Gauge, FolderSearch, Layers,
} from 'lucide-react'
import {
...
const TYPE_META: Record<CatalogAssetType, { label: string; cls: string; dot: string; icon: ReactNode }> = {
  ...
  data_model: { label: '数据模型', cls: 'bg-indigo-50 text-indigo-700', dot: 'bg-indigo-500', icon: <Layers className="size-3" /> },
...
export default function DataCatalogPage() {
  ...
  const typeCounts = useMemo(() => {
    const c: Record<CatalogAssetType, number> = {
      source_table: 0, semantic_object: 0, metric: 0, consumer: 0, vector_collection: 0, data_model: 0,
    }
    ...
    <div className="relative mx-auto flex w-full max-w-[1400px] flex-col gap-5 px-4 py-6 sm:px-6 sm:py-8">
      ...
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">数据资产</h1>
