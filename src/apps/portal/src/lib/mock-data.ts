...
/**
 * mockStreamingChatResponse — 模拟一个完整的流式 SSE 响应序列
 *
 * 在离线/降级模式下，模拟后端真实的流式返回效果。
 * 支持通过 AbortSignal 取消所有未完成的定时器。
 */
export function mockStreamingChatResponse(
  message: string,
  ...
  return new Promise<void>((resolve) => {
    ...
    safeSetTimeout(() => {
      ...
      onEvent({
        ...
        data: {
          ...
          confidence: 0.92,
          event_timestamp: ts(),
        } as StreamStartPayload,
      })
    }, 100)

    // ── 2. stream:intent_trace ─────────────────────────────────
    safeSetTimeout(() => {
      if (finished) return
      onEvent({
        event: 'stream:intent_trace',
        data: {
          request_id: requestId,
          intent: 'settlement_exception_guidance',
          confidence: 0.92,
          ...
          event_timestamp: ts(),
        } as unknown as StreamIntentTracePayload,
      })
    }, 300)
    ...
    safeSetTimeout(() => {
      ...
      onEvent({
        ...
        data: {
          ...
          message: '意图识别完成: 医保结算异常导办',
          request_id: requestId,
          event_timestamp: ts(),
        } as StreamStepPayload,
      })
    }, 500)
    ...
    safeSetTimeout(() => {
      ...
      onEvent({
        ...
        data: {
          call_id: callId,
          request_id: requestId,
          tool_name: 'query_transaction',
          params: { patient_id: 'P001', encounter_id: 'E001' },
          event_timestamp: ts(),
        } as ToolCallPayload,
      })
    }, 1400)

    // ── 7. stream:tool_result ────────────────────────────────
    safeSetTimeout(() => {
      if (finished) return
      const callId = 'call_' + Math.random().toString(36).slice(2, 10)
      onEvent({
        ...
        data: {
          call_id: callId,
          request_id: requestId,
          result: { settlement_status: 'failed', error_code: 'E001' },
          duration_ms: 600,
          event_timestamp: ts(),
    ...
    // ── 8. stream:delta × N ──────────────────────────────────
    // 根据消息匹配 mock 回复内容，拆分为 6 个 delta 块
    const mockLines =
      mockAIChatResponses[message] ?? [
        `我理解您的问题：${message}`,
        '',
        '后端服务当前不可用，已切换到离线演示模式。',
        '请启动 FastAPI 服务后重新尝试真实联调。',
      ]
    const fullText = mockLines.join('\n')
    const chunkCount = 6
    const charsPerChunk = Math.ceil(fullText.length / chunkCount)
    const deltaChunks: string[] = []
    for (let i = 0; i < chunkCount; i++) {
      const start = i * charsPerChunk
      if (start >= fullText.length) break
      deltaChunks.push(fullText.slice(start, Math.min(start + charsPerChunk, fullText.length)))
    }
    const deltaStartMs = 2800
    const deltaIntervalMs = 300
      ...
      safeSetTimeout(() => {
        ...
        onEvent({
          ...
          data: {
            ...
            request_id: requestId,
            event_timestamp: ts(),
          } as StreamDeltaPayload,
        })
      }, deltaStartMs + index * deltaIntervalMs)
    })

    // ── 9. stream:final ──────────────────────────────────────
    const finalTimeMs = 5000
    safeSetTimeout(() => {
      if (finished) return
      onEvent({
        event: 'stream:final',
        data: {
          ...
          event_timestamp: ts(),
        } as unknown as StreamFinalPayload,
      })
    }, finalTimeMs)
