...
function stageStyle(status: StageStatus, streaming?: boolean): string {
  let classes: string
  switch (status) {
    case 'pending':
      classes = 'border-slate-200/60 bg-slate-50/50 text-slate-500 dark:border-white/[0.04] dark:bg-white/[0.02] dark:text-slate-500'
      break
    case 'running':
      classes = 'border-blue-200/80 bg-blue-50/60 text-blue-700 dark:border-blue-400/15 dark:bg-blue-400/5 dark:text-blue-300'
      break
    case 'done':
      classes = 'border-emerald-200/80 bg-emerald-50/60 text-emerald-700 dark:border-emerald-400/15 dark:bg-emerald-400/5 dark:text-emerald-300'
      break
    case 'blocked':
      classes = 'border-amber-200/80 bg-amber-50/60 text-amber-700 dark:border-amber-400/15 dark:bg-amber-400/5 dark:text-amber-300'
      break
  }
  if (streaming && (status === 'running' || status === 'pending')) {
    classes += ' animate-pulse border-glow'
  }
  return classes
}

function stageIcon(status: StageStatus, streaming?: boolean) {
  if (status === 'done') return <CheckCircle2 className="w-3.5 h-3.5" />
  if (status === 'blocked') return <AlertTriangle className="w-3.5 h-3.5" />
  if (status === 'running' || (streaming && status === 'pending')) return <Loader2 className="w-3.5 h-3.5 animate-spin" />
  return <Clock3 className="w-3.5 h-3.5" />
}

export default function IntentTraceCard({ intentTrace, isStreaming = false, streaming = false, className = '' }: IntentTraceCardProps) {
  const [expanded, setExpanded] = useState(false)

  // Skeleton state: streaming started but no intent trace data yet
  if (streaming && !intentTrace) {
    return (
      <div className={`mb-4 ${className}`}>
        <div className="flex w-full items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-2.5 animate-pulse">
          <div className="flex items-center gap-2">
            <Loader2 className="w-4 h-4 text-cyan-400 animate-spin" />
            <span className="text-xs font-semibold text-white/40">等待意图识别...</span>
          </div>
        </div>
      </div>
    )
  ...
  // No data and not streaming — nothing to render
  if (!intentTrace) return null
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
              ...
              <div className="space-y-2.5">
                {stages.map((stage) => (
                  <div key={stage.id} className={`rounded-lg border px-3 py-2.5 ${stageStyle(stage.status, streaming)}`}>
                    <div className="flex items-center gap-2 text-xs font-semibold">
                      {stageIcon(stage.status, streaming)}
                      {stage.label}
                    </div>
