...
**门诊医保数据契约 P0（2026-08-28 补证）**：评审工作已执行；准入门禁与 P1 仍为 `blocked`。G01 内部锚点、G04 `o_FeeItem` 技术选型、`mzjyxx` v3 可查询模型及主要值域已通过语义层/源库只读核验；当前只处理业务口径与票据签认、缺失就诊时间/ID、CDC、只读权限、隐私阈值和正式签字，不再索要系统内已有资料或重复建设查询模型。
