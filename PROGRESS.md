...
| 2026-08-19 | **Issue #19 单元医疗类别区分**：新增确定性分类器（就近原则+最长别名，回退「通用」）；提取 intake 落 `unit_med_type` 并回填规则 med_type（空值继承、已有归一）；审核页新增按医疗类别筛选。T1 8+557、T2a 1+51、T2b 1+1、前端 29+138、tsc/build 通过；失败均为预存环境依赖（stash 对照）。存量数据重提取后获得分类 | 政策知识管线；需求迭代记录 Issue 19 节；领域字典新增 med_type_classifier |
