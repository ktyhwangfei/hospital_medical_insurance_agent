...
**Workflow 治理配置化 + 院区隔离接缝（部署形态 A：一院一套部署，2026-09-17 完成，待验收）**：针对「一套产品多院、我们维护」的成本模型，把院区差异化从代码/环境变量搬到治理配置层。①**关键词与启停入治理库**——新增 `WorkflowConfigOverride`（按 `workflow_id + hospital_code` 唯一）+ 存储四件套 `storage/workflow_config/`（CREATE+ALTER 双写 DDL，内存回退）+ `runtime/workflow/config_service.py` 三层解析（代码默认 ← 全局覆盖 ← 院区覆盖，`WORKFLOW_DISABLED` 环境变量保留为故障下线 kill-switch 且优先级最高）；路由/模式入口/目录统一改读生效配置，写入后缓存失效同进程立即生效（无需重启）；新增 `PUT /workflow-catalog/workflows/{id}/config`（JWT `workflow:write`），目录回显 `keyword_source`；写入语义：省略 `intent_keywords` = 保持该行词表（只改启停不误清关键词），显式 null = 回落代码默认。②**院区隔离接缝**——`SemanticDataset.hospital_code`（JSONB 存储无需 DDL；空=平台通用，非空=该院专属）+ `semantic_layer/guard.py` 规划期 fail-closed（跨院区绑定即拒，转译为 `SemanticQueryPlanningError` 保 API 4xx 契约）+ `shared/hospital_scope.py` 部署级院区身份（B 形态改请求级取值即可）。**明确不做**：同对象多院各一份语义绑定需按院区版本化发布或 `dataset_code` 间接层，本轮不动（避免未演练抽象）。验证：新增单元 9 例 + API 6 例；全量 unit 2828 passed（2 失败经干净检出 6962b32 复现为 policy_rules_search / 启动脚本断言预存）；相关 API 61 passed（1 失败 `tool_query_flow_metrics` 缺输入契约经干净检出确认为 main 预存）；Flow 失败集与 main 基线完全一致（25 项环境依赖）。领域字典 §3 已同步。

**Issue #69 产品化第二轮：多院映射层 + 入口契约 + Golden（本工作区 tool-69，2026-09-16 完成，待验收）**：需求方确认多院区产品定位（各院单独接数据源，功能代码零改动）且四问无原始单据。落地：①**D1 映射层**——四记录查询去 schema 硬编码，`RecordQueryMapping`（表/列/方言 + 部分覆盖合并，默认 bjybdb 基线）+ PG 表 `record_query_mappings` + 登记脚本 `scripts/set_record_query_mapping.py`，换院=登记 JSON 零代码，行投影与 SQL 渲染全部经映射（含二档 PG 方言引用）；②**D2 入口契约**——`PolicyQARequest` 新增 `settlement_ids`/`id_card`/`visit_date`（Q1 多单号、Q2/Q4 门诊退费身份入口，id_card 不落轨迹/日志），上下文默认值化贯通 mode/关键词两路，退费步骤接身份入参；③**退费三链路 + 诚实边界**——住院 djh→tflydjh / HIS 交易号→退费交易对（关联原交易起付线与年度累计，Q4 事实）/ 身份+日期→门诊 HIS；门诊医保结算无身份时显式声明不可核对（不再偺无退费结论）；顺带修复退费 SQL 基础 OR 条件未括号致过滤被优先级吞掉的真实 bug；④**D6 停用开关**——`WORKFLOW_DISABLED` 环境变量逐条下线（路由/模式/目录/Portal 徽标四处一致）；⑤**D4 Golden 同型替代**——四问从基线库挖同型数据（Q1 同患者 55 单同码取 687/689/691；Q2/Q4 交易族 007/008/009 含退药负数量明细+同日重开；Q3 djh=687 特病 502+大病 5161.16+救助 16756.09），`TOOL69_LIVE_DB=1` 实库回归 4 例。验证：无库聚焦 56 例 + 实库 golden 4 例 + Vitest 6 例 + tsc 零错误；全量 integration/api 504+ passed（仅剩 infra_skill 2 例预存）。仍待上线闸门：生产库 schema 验证、身份字段前端输入 UI、答案 LLM 组装/citations、治理发布流、监控容量基线。

**Issue #72 Workflow 节点化架构合并 + #69 Tool 能力适配（本工作区 tool-69，2026-09-16 完成，待验收）**：将 workflow-69 工作区未提交的 #72 代码（《Workflow 自研节点开源调研与落地方案 V1.0》，docs/research/）合入本工作区并按其设计改造 Tool 层：①节点分类——WorkflowStep 判别联合拆为 ToolNode/DomainNode/DecisionNode/OutputNode，执行器支持白名单领域处理器（Pydantic 契约 fail-closed）、前向分支与输出节点；②**对比计算下沉领域节点**——calc_tools.py 删除，settlement_policy_compare/evidence_completeness/evidence_merge 入 DOMAIN_HANDLERS，同药跨单对比按设计拆为 tool_get_fee_detail（批量取数 Tool，双入口 v1.1.0）+ same_drug_compare 领域节点（纯函数契约校验）；③新增三工作流按节点模型重写（退费核对=3 Tool+Output、同药跨单=Tool批量取数+Domain对比+Output、待遇叠加=2 Tool+Output）并全部加入 KEYWORD_ROUTED_WORKFLOWS（政策问答/问数不参与关键词 fallback，顺带修复上轮预存的 4 个 broad-router 路由测试）；④Tool 步骤显式 input_mapping 替代整包透传。合并中修复：ToolRegistryService 支持注入独立存储（flow 测试 stub 以 created_at 压过内置版本污染同进程用例，先红后绿）；tv_parse_data_query_intent 升 1.2.0（共享 PG 已有他人 1.1.0 无契约版，内容变更未升版冲突）；_collect_uncertainties 补收输出 uncertainties 键（低保缺失声明不达公开结果违反来源可追溯硬约束）。验证：单元/flow/API 聚焦 51 例 + Portal Vitest 6 例 + tsc 零错误全绿；全量 integration/api 仅剩 2 预存（infra_skill，干净 HEAD 已甄别）；真实 bjybdb 冒烟：Q1 同药跨单（tool→domain→output 三类节点真实链路）status=complete、差异正确标记；Q3 待遇叠加含低保 uncertainty 传播。前端 /tools 页合并字段契约展示（ToolFieldTable+执行细节）与节点链渲染（NODE_LABELS/领域节点/输出节点），api.ts DTO 双向对齐。

**Issue #69 Tool 真实能力接入（本工作区 tool-69，2026-09-16 完成，待验收）**：评审确认四个真实用户问题满足度 0/4 后，基于同日 SQL Server 盘点（`docs/reviews/2026-09-16-tool69-real-capability-vs-4-questions.md`：退费链路/费用明细/人员身份/待遇叠加在源库均有真实数据，此前"无数据源"判断实为 PG 落地子集丢字段）落地四能力：①`tool_get_refund_record` 绑定真实实现（住院 djh→tflydjh 医保端链路 + 门诊人员身份→HIS o_Trade 退费链路，`RefundRecordPort` + `settlement_record_queries.py` 适配器 + `settlement_record_lookup.py` 组合根）；②`tool_resolve_settlement_by_person` 绑定（o_Trade P_IDNo/P_ICNo + yb_brdjxx sfz/kh 双源，多候选强制澄清）；③新增 `tool_get_fee_detail`（yb_zyfymx/yb_mzfymx，NATION_CODE 同药对齐键 100% 填充）与 `tool_compare_same_drug_across_settlements`（纯函数对比单价/数量/医保内占比/先行自付，归因落 uncertainties）+ `tool_get_benefit_stacking`（特病登记 tsb1-3 + 结算表逐笔 BIG_ILL_PAY/CIVIL_IN 分摊事实，低保 dbzbs 无数据即声明）；④新增 `wf_settlement_reimbursement_diff`（Q1）与 `wf_benefit_stacking_attribution`（Q3），退费核对链加费用明细步骤。顺带修复预存缺陷（先红后绿）：invoke 未按签名过滤参数致固定签名工具 TypeError、parse_data_query_intent 缺契约、_collect_conclusion 取中间步骤摘要。验证：单元（新增 12 例）+ API 45 例 + 相关目录 224 例全绿；全量 integration/api 500 例通过、6 例失败经干净 HEAD 检出甛别为预存；真实 bjybdb 冒烟五链路全通（同药对比正确标记含负数量冲同行的差异、人员定位多候选强制澄清、门诊退费真实命中 HIS 退费交易）。未做：费用明细语义对象、PG 落地视图扩列、T7/T8 权威规则取证、住院退费真实数据验证（tflydjh 测试库全 0）。
**数据治理体系 V3.0（2026-09-15，进行中，无 issue 编号）**：架构设计《数据资产-语义模型-数据治理体系架构设计-V3.0》已定稿并获确认（物化 view 注册为普通 dataset、query_planner 不改；页面优先）。**Slice 1 + Slice 6 已完成**：
- **Slice 1 数据建模后端** ✅：`src/domain/data_model/`（DataModel 聚合根 + DataModelField + DataModelMapping 多源映射，粒度/角色/派生依赖校验）+ 存储四件套（`data_models` 文档 JSONB 表 + `data_model_mappings` 行级表，真实 PG 已建表）+ `DataModelingService`（draft→published→deprecated 状态机，published 结构冻结，映射 draft→confirmed 确认流）+ API `/data-governance/models`（复用 data_governance:read/write 权限，404/409/422 错误码）。Unit 16 + API 12 passed；领域字典已登记 §14.13。
- **Slice 6 Portal 一级模块补齐** ✅：数据治理中心 tabs 与资产生命周期六阶段一一对应——运行概览/数据接入/同步任务/数据探查（新页 `/profiling`，按表聚合字段画像只读视图 + 跳发现中心扫描）/数据建模（新页 `/modeling`，模型 CRUD + 字段结构编辑器 + 映射确认流）/数据加工（`/flows`，自 /flow 迁入）/质量与发布（新页 `/quality`，同步质量门 + Flow 发布证据聚合，纯前端复用既有接口）/数据资产（`/assets`，自 /data-catalog 迁入并更名「数据资产」，旧 /data-catalog 301，顶部导航撤掉独立「数据目录」入口）。概览页为「语义标准横条（横向贯穿，非阶段）+ 生命周期流程条」两段式；术语统一对齐 V3.0 §1.1 字典。验证：Portal Vitest 75 文件 552 passed、tsc 零错误、build 通过；本工作区实测 `/data-governance/models` 无 token 401（鉴权接线）、PG 两表结构逐列一致、三新页 200。
- **架构修正记录**：首版流程条曾把「语义建模」误作流水线阶段，违背「语义层横向贯穿」原则，已修正并记入设计文档 §1.1。
剩余：Slice 2 Flow 物化模式（materialize_model → dwd view）、Slice 3 指标 model_field_ref 绑定、Slice 4 语义映射确认流（discovery 拆分）、Slice 5 Settlement 闭环、Slice 7 SQL Trace 可视化。

**端到端全流程联调（2026-09-15 深夜，明晨验收）**：门诊数据全链路真实跑通——①数据接入：清除 4 个他工作区 sync worker（共享 PG 互抢任务陷阱再现），本工作区单 worker，任务 failed→running，对账批次成功，SQL Server o_Trade 592/o_FeeItem 2139 行正常，PG mz_trade 588/mz_fee_item 2103 行；②数据探查：bjybdb 全库扫描 356 表/5833 字段/313 已映射；③数据建模：dwd_mz_settlement（门诊结算明细，粒度 trade_no，22 字段/22 映射全 confirmed/published v2，映射对齐 Golden Flow 签核口径）；④加工+质量：query_by_metrics 全链通过，勾稽门 identity 断言真实数据成立（6643.69=113.66+6530.03）；⑤数据资产：目录刷新 160 资产/269 列/230 边；⑥消费：运营分析六指标与 Golden Flow 数值逐字一致；可信问题库 /match 命中+execute 回放 complete；**Agent Hub data_query 端到端**：「门诊统筹基金支付是多少」→LLM 意图解析→Flow 消费契约→回答「门诊统筹基金支付金额：113.66（质量 passed）」complete/verified。

**联调修复的四个真实缺陷（缺陷驱动铁律，先红后绿）**：㈠ WorkflowExecutor 只捕获 ToolInvocationError，工具内部领域异常穿透崩 SSE 生成器致流静默断开——加兜底捕获降级 unavailable；㈡ WF_DATA_QUERY 通道错配：意图解析器构造 SemanticQuery 单笔锚点查询（anchor 写字面量 entity_code），运营聚合问法必抛「锚点未登记」——改 Flow 消费契约通道：新 tool_query_flow_metrics（query_by_metrics，conclusion 用指标中文名+citations 携带发布证据），意图解析目录改为已发布 Flow 消费契约指标，ALLOWED_TARGET_REF_PREFIXES 增补 src.runtime.flow.；㈢ parse tool 定义变更触发 ToolVersionConflictError（#69 防静默覆盖正确拦截）——升版本 1.1.0；㈣ fetch_settlement 步骤缺 input_mapping 致全 context 穿透 TypeError（#69 预存 bug，生产上一直靠 fallback 掩盖）——补 input_mapping。ToolRegistryService 增 storage 注入 seam（测试隔离）。验证：workflow+tool_registry 24 passed、workflow_routing 31 passed、后端 unit 2778 passed（2 失败 main 预存）、integration/api 490 passed（6 失败 Milvus/LLM 环境预存）、前端 Vitest 552 passed + tsc/build 通过、15 页面实测 200。

**环境配置**：.env 固化 AUTH_JWT_SECRET + DATA_SOURCE_MODE=real_db（启动脚本注入）；MODEL_BASE_URL/MODEL_API_KEY 从 main 检出拷贝（Agent Hub 意图解析依赖）。

**明晨验收入口**（本工作区 8133/3133）：http://127.0.0.1:3133/data-governance 语义标准横条+生命周期六阶段；/data-governance/modeling 可见 dwd_mz_settlement（published，22 字段+22 已确认映射）；/data-governance/profiling 表字段画像；/data-governance/quality 勾稽发布证据；/policy-qa 问数 Tab 问「门诊统筹基金支付是多少」应答 113.66 元。

**选表同步通道 + 六表全流程（2026-09-16 凌晨）**：
- **环境修复**：共享 PG 凭据被多工作区 bootstrap 反复重封（local-environment-bootstrap）——统一所有工作区 DATA_GOVERNANCE_MASTER_KEY 为主检出 key（bootstrap 探测 healthy 即不重封），清理 12 个他工作区孤儿进程，本工作区单 worker 独占。教训：start-servers.ps1 每次启动都跑 bootstrap，多工作区必须同 key 才不互炸。
- **选表同步（新通道）**：`src/data_platform/table_sync.py` + `storage/table_sync/` + `table_sync_executor.py`（探查列→DDL 对齐→单事务全量覆盖）+ API `/data-sources/{id}/sync-tables`（选表/清单/立即同步/移除）+ worker 集成（主契约任务后顺带执行，失败隔离）。真实同步四表：yb_brdjxx 140 行、yb_mzjyxx 33 行、yb_mzfymx 113 行、o_diagnose 619 行（用户列的 by_brdyxx 实为 yb_brdjxx 笔误）。pyodbc Row 转 tuple 修复。
- **六表建模全部 published**：dwd_mz_settlement(22 字段) / dwd_mz_fee_item(12) / dwd_mz_diagnose(7) / dwd_yb_patient_reg(8) / dwd_yb_mz_settlement(14) / dwd_yb_mz_fee_item(12)，映射全部 confirmed。发布门槛修正：纯维度模型（诊断/登记）无 fact 合法化。
- **医保加工 Flow**：flow_yb_mz_settlement 已发布——医保门诊基金构成 8 指标（总金额/统筹支付/统筹自付/大额支付/大额自付/现金/账户/笔数），口径句诚实标注「全量直通聚合，正式签核口径待补」（ptybn/ptybw 实测全零未用；zje 与支付段和差 2793.65 未发明勾稽门）。query_by_metrics 实测 356770.97/128759.64 等与手工核对逐字一致。Agent Hub 问数「医保门诊统筹支付一共多少」→ 128759.64 元 complete。
- **数据资产**：目录新增 data_model 资产类型 + 选表落地表 source_table 资产 + 血缘边（落地表 feeds 数据模型，6 链全成）——刷新 170 资产/344 列/236 边。前端资产页 TYPE_META/筛选 tab 同步。
- **前端**：tabs 重排对齐生命周期（接入→探查→同步→建模→加工→质量→资产）；数据接入页双模式卡（CDC 全表对接=占位待 DBA / 探查后选表同步=当前可用）；探查页表级「加入同步/立即同步」；数据同步页（原同步任务）新增选表同步区。
- 验证：后端 unit（table_sync 5 + data_model 16 + domain/catalog 104）+ API（sync-tables 2 + data-model 12）passed；前端 Vitest 78 文件 552 passed + tsc/build 通过；全部治理页面实测 200。
- **验收前展示修复（2026-09-16 凌晨）**：①「数据同步看不到选表」根因=前端 dev 进程崩溃致 /api 代理无响应（非数据问题），ws.ps1 restart 恢复；②「数据探查看不到全部表」根因=多链路共享 discovery_scan_tasks 单 latest 缓存，policy_milvus/outpatient_postgres 的小扫描覆盖了 HIS 全库扫描结果——`get_latest_result` 增加按数据源隔离参数（`/discovery/results?datasource_id=`），探查页按当前健康数据源取数，实测恢复 356 表/5833 字段；③探查页展示优化：统计条（表数/字段数/已映射/已选同步）+ 表名字段名搜索。psycopg `%s IS NULL` IndeterminateDatatype 陷阱：改 Python 侧分支而非可空参数。
- **探查页对齐发现中心（2026-09-16 晨）**：发现中心 1900 行页面提取为共享组件 `src/components/discovery/discovery-center.tsx`（新增 `mappingBaseUrl` prop），语义层发现页变壳复用；数据治理探查页重构为「选表同步面板（按表聚合画像/搜索/加入同步/立即同步）+ 发现中心全能力嵌入」，「纳入数据模型/批量映射」跳转指向数据治理内 `/data-governance/modeling`；数据接入页「数据探查」按钮改跳治理内探查页（全部跳转不出数据治理中心）。FieldExpandDetail/QuickMetricForm 为预存死代码（记录不删）。前端 Vitest 78 文件 554 passed + tsc/build 通过，三页实测 200。
- **探查页最简化重构（2026-09-16 午，易维护导向）**：撤掉发现中心嵌入（1900 行重型组件耦合过高），探查页自包含重写（~300 行，仅依赖 results/scan SSE/sync-tables 三个接口）：控制条（数据源/重新扫描简化 SSE/上次扫描/统计）+ 筛选条（搜索/全部/已选同步/含未映射）+ 表清单（聚合画像/主键/同步状态/加入同步/立即同步/展开字段画像）+ 展开行「纳入建模」跳 `/data-governance/modeling` 带预填（建模页 useSearchParams 预填字段行）。DiscoveryCenter 组件回归语义层独享（发现页壳不变）。测试重写 5 例；全量 78 文件 555 passed + build 通过。
- **拷问后加固轮（2026-09-16 晚，grilling 两轮结论落地）**：
  - **治理动作有治理**：Flow 发布 published_by 从 JWT 认证主体取（废弃自报字段，publish 端点加 data_governance:write 验签）；数据模型发布加 pending_review 评审步（draft→pending_review→published，直接发布 422）；选表同步记事件表 `data_source_sync_events`（select/remove/run 留痕操作人）。
  - **Q10 双通道收敛**：o_Diagnose 从选表通道摘除（契约管道 outpatient_diagnosis_current 已覆盖，有质量门），dwd_mz_diagnose 退役重建（映射改指定制管道落地表，移除未采集的 doctor_name 字段）——模型「deprecated 可同 code 重建」机制为此新增。
  - **P1 关联键推断**：yb_mzjyxx.SETL_ID ↔ mz_trade.T_FeeNo（医保结算 ID=HIS 收费号，420/588 行命中 71%）；医保侧 djh 内部全关联（33/33）；jylsh↔T_TradeNo 不连通。标注待业务确认。
  - **Slice 2 Flow 物化**：FlowDefinition 加 materialize_model 可选字段；发布时按模型 confirmed 映射物化 dwd 直通明细视图（多源 UNION ALL）+ 注册语义 dataset（object_code 取 Flow 源契约）+ fail closed。真实链路：flow_yb_mz_settlement rev5 发布 → dwd_yb_mz_settlement 视图 33 行可查。
  - **Slice 3 指标绑定（登记层）**：Metric 加 model_field_ref（DDL 双写+读写映射），回填 40 个门诊指标（含 o_Trade→mz_trade 契约桥解析）；查询路径切换随对象版本发布留后续。
  - **e2e 验收脚本** `scripts/e2e_governance_closed_loop.py`：七段 17 项断言全过（含物化视图+指标绑定）。
  - 验证：后端 unit 2796 passed（2 main 预存）+ API 50 passed；前端 79 文件 561 passed + tsc/build 通过；评审流真实链路（直接发布 422→提交评审→发布→退役）实测通过。
- **拷问第三轮加固（2026-09-16 晚，验收隐患消除）**：①Flow 编辑器属性面板加「物化目标数据模型」下拉（Slice 2 前端暴露）；②token 有效期 8h→24h + 重启重计时（验收日不过期）；③探查页表清单分页（50/页，356 表不卡）；④建模页详情加「物化状态」区（模型→Flow→视图链路可见）；⑤质量与发布页加「源库对照」工具（compare-source API 白名单双库同口径；实测 yb_mzjyxx 总金额 356770.97=356770.97 零差异自证，o_Trade 592 vs 588 差 4 行=质量门拦截活证据）；⑥各 tab 页底部「下一步」引导卡片（生命周期闭环引导）。前端 79 文件 561 passed + build 通过；e2e 验收脚本全绿。
- **验收 bug 修复（2026-09-16）**：①o_Diagnose 回归选表通道——Q10 收敛过头（定制管道 children 不支持按 RecipeDate 独立增量），已按 RecipeDate 增量同步（619→598 行主键去重，水位线推进）；时间字段判断逻辑=类型筛选+人工确认（探查页弹窗）。②运行概览重构：移除 CDC 列（未配置不展示），同步列按通道展示（契约管道模式 + 选表同步表数），消除混乱。
...
| MCP 工具管理 | 3 | 0 | 3 | 0 | 0 | — |
| 知识库管理 | 7 | 1 | 6 | 0 | 0 | §2 P9 5 tab 已上线；语义提议 S1 已完成 R4，S5 冲突维度候选已完成聚焦验证；6.7 Issue #33 上线补强已完成，门禁 3 项中 1 项达标（待需求方判定）；2026-09-16 迭代 22 知识体系页（Milvus 集合盘点+维度路线知识树）已上线 |
...
| 6.3 | 政策知识浏览 | `policy-knowledge/*`（P9 5 tab；迭代 22 新增知识体系 tab） | `policy_knowledge_routes.py` | impl_done |
...
| 2026-09-11 | **Issue #51 P1-5 LLM 智能诊断落地（citations 强制，无来源不落结论）**：单条 finding 一键生成带证据引用的根因诊断与分级建议。**领域** `src/domain/ops/models.py` 扩充——`DiagnosisStatus`（complete/insufficient_evidence）、`DiagnosisActionLevel`（L1 自动白名单/L2 人工确认/L3 禁止，建议仅指引执行仍受 #53 白名单约束）、`DiagnosisCitation`（citation_id+source+quote，**只能从证据目录选取、quote 取自目录原文，模型不可编造**）、`DiagnosisAction`（未挂任何有效引用的建议构建时丢弃）、`OpsDiagnosisReport`（存 `OpsFinding.diagnosis` 最新覆盖，带 model_route 审计）、`DiagnosisUnavailableError`。**服务** `src/runtime/ops/diagnosis.py` `OpsDiagnosisService`：证据目录 `build_evidence_catalog`（payload 逐字段 `payload.*` + 定向补充证据 `EvidenceCollector` Protocol——默认实现为 data_sync_failed 附 `OutpatientGovernanceStore.list_attempts` 最近 5 次尝试（safe_error_code/safe_message/rows）；统一过 `security/desensitization.redact_sensitive_text`，PHI 原值不进模型不落库）→ ModelGateway scene=`asset_diagnosis` model_type=`llm`（system prompt 明令只依据编号证据、输出严格 JSON、证据不足输出空 citations 不得硬编结论）→ `build_report` 校验：引用按目录映射去重、编造编号（E99）丢弃、非法级别/无描述/无有效引用的 action 丢弃；**硬约束：引用为空 → status=insufficient_evidence、root_cause 置空、actions 清空，不驱动任何修复动作**；模型失败/输出不可解析（容忍 markdown 围栏与前后杂文本）抛 `DiagnosisUnavailableError` 不落库不覆盖旧报告。**存储** `save_diagnosis` 覆盖写 diagnosis 列不动 status/revision（诊断只读），端口/PG/内存三处齐备。**API** `POST /ops/findings/{id}/diagnose`（ops:write；404 FINDING_NOT_FOUND / 503 DIAGNOSIS_UNAVAILABLE），`get_ops_diagnosis_service` 依赖注入 seam。**Portal** 详情抽屉「发起诊断」按钮（sky 主题）+ 诊断报告卡片：根因段、证据引用 `<details>` 可展开（编号+来源+原文）、分级建议（L1 emerald/L2 amber/L3 red 徽标+引用编号）、不确定性 amber 醒目块、insufficient_evidence 专属态（AlertTriangle「证据不足，未生成结论与建议」不渲染根因与建议）、model_route/生成人/时间审计脚注；`ops-api.ts` DTO 对齐 + `diagnoseOpsFinding`。**顺带修复两件**：① #40 缺陷——`ops_weekly_summary` 曾用 model_type=`"chat"` 但路由表只有 `(default, llm)` 兜底，未发布治理路由时 `ModelRouter.resolve` 抛 ModelRouteError → AI 摘要恒降级；改 `llm` 并为 `ops_weekly_summary`/`asset_diagnosis` 加显式路由条目（Unit 回归守护）。② 预存时间炸弹测试修复——`test_ops_remediation.py`（4 失败）与 `test_ops_api.py`（3 失败，#40 会话已发现）：夹具 `next_run_at` 固定 2026-09-09，而巡检/修复验证用真实时钟判滞后，日历推进后健康任务被判 lagging；改相对当前时钟取未来值（#40 会话遗留的 3 个预存失败就此关闭）。验证：Unit 13（complete 报告引用取自目录/无引用 insufficient_evidence 负例/编造引用丢弃/action 四规则丢弃/围栏 JSON 容忍/证据目录 PHI 脱敏/服务落库不动 status·revision/模型失败不落库/两 scene 路由解析回归）+ API 6（401/403、complete 落库+revision 不动、insufficient_evidence 无动作、503 不落库、404）+ Flow 活库 1（JSONB 报告往返/重诊覆盖 insufficient_evidence）全绿，ops 全量回归 117 passed；portal vitest 抽屉 15（含发起诊断回填只读不刷列表/insufficient_evidence 专属态/503 错误条/只读无入口）+ tsc 0 错误 | #52 P1-6 定时巡检调度可开工 |


| 2026-09-11 | **Issue #52 P1-6 定时巡检调度落地（复用门诊同步 claim_due_job 单进程 PG 模式，不引入 Airflow/Temporal/celery）**：**存储** `ops_inspections` 运行记录表（trigger_source/status/triggered_by/起止时间/finding_count/new_finding_count/checker_errors JSONB，CREATE+ALTER 双写 DDL + started_at 索引）+ `ops_inspection_schedule` 单行调度表（schedule_id=1 CHECK 约束，next_run_at + active_inspection_id 互斥位；bootstrap ON CONFLICT DO NOTHING 首行 next_run_at=now → worker 首轮立即巡检）+ 端口 5 方法（claim_inspection/complete_inspection/get_inspection/get_latest_inspection/get_inspection_schedule），PG/内存实现齐备；**claim 抢占语义**：`SELECT ... WHERE active_inspection_id IS NULL AND (next_run_at <= now OR trigger=manual) FOR UPDATE SKIP LOCKED` 事务内 INSERT running 行 + UPDATE 调度行，败者得 None——手动触发绕过到期检查、并发恰一胜出（活库双连接验收）。**调度器** `src/runtime/ops/scheduler.py` `OpsInspectionScheduler`：run_manual（被抢占抛 `InspectionInProgressError`→API 409 INSPECTION_IN_PROGRESS）/run_scheduled_once（未到期返 None；异常只记日志不抛）/get_summary；手动与定时完成统一推 next_run_at=完成时间+周期；灾难性失败落 FAILED 运行行（checker_errors 只存安全文案「巡检执行异常，详见服务端日志」，异常原文可能含连接信息不落库）+ 顺延一整周期；周期取 env `OPS_INSPECTION_INTERVAL_MINUTES` 默认 1440（每日），非法/<1 回退默认。**领域** `OpsInspectionTrigger`(manual/scheduled)/`OpsInspectionStatus`(running/succeeded/failed)/`OpsInspectionRun`/`OpsInspectionScheduleState`/`OpsInspectionSummary` + `OpsCheckerError`（自 service 迁入领域共享）；`OpsInspectionResult` 增 inspection_id/trigger_source 回填与 `new_finding_count` computed_field（=occurrence_count==1 首见问题数，复现累计不算新发现）。**worker** `scripts/run_ops_inspection_worker.py`（--once/--status/--poll-interval 5-600 默认 30s，SIGINT/SIGTERM 优雅退出）+ start-servers.ps1 [3b/5] 段启动并写 `ops_worker_pid` 状态、stop-servers.ps1 按 CommandLine 校验杀进程。**API** POST /ops/inspections 改走调度器留痕 + 新增 GET /ops/inspection-summary（ops:read）。**Portal** /ops 顶部巡检摘要条（最近巡检时间+状态徽标+触发方式+新发现数 / 下次巡检时间 / 周期文案 每天·每小时·每 N 分钟；in_progress 显「巡检进行中…」、无记录显「尚无巡检记录」），巡检完成 note 增「新发现 N 个」。验证：Unit 17（周期 env 解析 4/手动留痕与抢占/定时到期与未到期/失败留痕与顺延/summary 三态/8 线程 Barrier 并发恰一胜出）+ API 38（新增巡检调度类：手动 run 字段与 new_finding_count 递减、409 进行中、summary 鉴权与形状）+ Flow 活库 8（新增 2：**双连接 Barrier 并发 claim 恰一胜出+complete 释放+定时未到期 None+手动抢入 FAILED 留痕**；scheduler 全链路 roundtrip——scheduled 认领/next 推进/summary 聚合/二次未到期/manual 回填）全绿，ops 全量回归 102 passed；portal vitest 全量 488 passed（ops 页 12：摘要条三态/409 错误码展示/新发现 note）+ tsc 0 错误 | #54 P2-8 L2 人工确认修复流可开工 |
| 2026-09-11 | **Issue #54 P2-8 L2 人工确认修复流落地（复用 task_closure waiting_human_confirmation 任务，不新建确认机制）**：非白名单/诊断 L2 建议的问题转人工处理，完成后自动复检回链。**领域** `src/domain/ops/models.py`——`OpsFindingStatus` +`waiting_human`；`OpsFindingEventType` +`manual_requested`/`manual_completed`；`OpsFindingDetail.manual_task` 最新任务投影；`OpsManualTarget`（policy_knowledge/skill_draft/external）+ `manual_target_for_asset` 按资产映射（knowledge→政策知识治理页、skill→技能草稿流程、data/runtime→外部系统）；`OpsManualHandoff`/`OpsManualResult` 投影模型 + `ManualTaskNotFoundError`。**服务** `src/runtime/ops/service.py`：`request_manual_handling`（open→waiting_human；**版本先验→流转→建任务**写入次序，冲突/任务失败均不留悬空任务；task_closure `create_task(status=waiting_human_confirmation, workflow_id=finding_id)` 反查）；`complete_manual_handling`（任务→completed 回填 output_data.handled_by/result_note/handled_at；**复用 #53 `_verify_remediation` 自动复检**：通过→resolved、仍报→回 open 按最新草稿 upsert 复现累计、检查器异常→回 open 待下次巡检；重复完成幂等回读）；`reopen_finding` 扩语义 waiting_human→open（撤回人工处理）；remediate/ignore 均要求 open——**未完成人工确认前问题不得进入 resolved（验收负例）**。**API** `POST /ops/findings/{id}/manual-handoff`、`/manual-complete`（ops:write，body note 选填/result_note 必填 + expected_revision 乐观锁；404 MANUAL_TASK_NOT_FOUND、409 FINDING_TRANSITION_INVALID/FINDING_REVISION_CONFLICT），详情 GET 携带 manual_task。**Portal** 详情抽屉：诊断报告含 L2 建议的开放问题出现「转人工处理」入口（amber 主题+选填说明表单）；waiting_human 状态徽标「转人工处理中」+ 列表筛选项；**人工任务卡片**——跳转治理页链接（/policy-knowledge、/skills，external 仅文案）+ 发起人/时间/说明 + 完成登记表单（result_note 必填）+ 已完成回链展示处理人/时间/结果；时间线 manual_requested/manual_completed 标签；waiting_human 下重开按钮文案「撤回人工处理」。**验证**：Unit 新增 15（test_ops_manual_flow：目标映射/发起含版本冲突无悬空任务/完成三分支/幂等/守卫负例）；API 新增 8（鉴权 401/403、状态机正反例、remediate+ignore 双负例、幂等）；Flow 活库新增 1（人工任务真实落 tasks 表、完成回填 JSONB 往返、复检 resolved、tasks 行清理）；ops 全量回归 Unit 71 + API 40 + Flow 9 全绿；portal vitest 全量 494（drawer 21：L2 入口门槛/转人工/外部无链接/完成回链/409 冲突/只读）+ tsc 0 错误 | P1-5/P1-6/P2-8 三 issue 全部完成，待统一页面验证 |
| 2026-09-16 | **迭代 22：知识体系页面——Milvus 结构化/向量化知识框架化展示（docs/steering/政策知识治理-需求迭代记录.md 迭代 22）**：**后端** `GET /policy-knowledge/knowledge-map`（`policy_knowledge_routes.py`，`_resolve_knowledge_map_sources` 为测试注入 seam）——Milvus 集合盘点（`policy_rules_*`/`policy_facts_*` 分类 + row_count + active 读路径标记，active 判定复用 `release_resolver.resolve_rules/facts_collection` 不另立口径）+ active 规则集合 24 字段全量投影（`_row_to_dict` 解包 FieldTrace 裸值，含 payment_ratio/deductible_amount/cap_amount/rule_value/source_text）+ facts 按 doc_id 聚合分布；pymilvus 缺失/连接失败 503 MILVUS_UNAVAILABLE，facts 分布失败仅降级为空。**前端** Portal `/policy-knowledge/knowledge-map`（导航「知识体系」页签）：汇总条（规则/事实/集合/覆盖文档）+ 集合盘点表（当前读路径 emerald 标记 + facts 文档分布 chips）+ **维度路线可调知识树**（9 维度 chips 编辑，默认参保体系→医疗类别→医院等级→人群→规则类型，localStorage `policy-knowledge-map-route-v1` 持久化带合法性校验回退）+ 叶子规则卡片（类型/发布状态徽章、比例/金额段/有效期 MetaChip，1900-01-01/9999-12-31 哨兵不渲染，原文 line-clamp-2）；聚合在前端一次投影完成，页面只读零写操作。**顺带修复**：layout `getActiveTab` 原 startsWith 首匹配被 `/knowledge` 前缀吞掉 `/knowledge-map` 高亮，改最长前缀匹配。**验证**：API 6 passed（FakeMilvusClient：盘点 active 标记/FieldTrace 解包/facts_by_doc 聚合/空集合/facts 失败降级/ImportError 503）；portal 新页 5 passed（盘点渲染/树聚合两步展开到叶子+哨兵日期不出现/路线持久化/空态/失败重试），policy-knowledge 前端全量 162 passed、api client 17 passed、tsc 0 错误 | 发布卡片「规则总数：暂无统计」硬编码与规则引用统计待后续迭代吸并 |

- **数据专家拷问轮（2026-09-17，七项全批落地）**：
  - Q3 落地表唯一性仲裁：契约覆盖表选表注册需显式 purpose=reference（禁止建模、不入安全边界）；存量 o_Trade/o_FeeItem 已标 reference；仲裁实测生效（无声明注册被拒）。
  - Q1 增量适用性警告：time-candidates 返回列元信息（最大值/非空率）+ 审计时间列检测 + 警告文案（无审计列=只捕获新增不捕获存量变更）；选表弹窗展示。
  - Q2 配置卫生：/config-health 检出 6 项「有时间字段但全量模式」待归因；同步页顶部警告条。
  - Q7 数据截止时间：/data-cutoff 安全分析边界=min(治理表水位线)，实测=2025-08-07（受限于 yb_zyfymx）；同步页展示边界+各表水位线徽章（最落后表标红）。
  - Q5 质量规则实质化：ops 新增 governed_table_quality 检查器（值域漂移/负数金额），复用 finding 机制。
  - Q6 字段级血缘：data_model_mappings 落为 field_map 边（74 条），relation 扩至 128。
  - Q4 元数据主从契约冻结：数据模型=结构 master，语义层=派生消费面，check_model_semantic_consistency() 一致性校验（实测检出 dwd_yb_mz_settlement 两处漂移）。写入 V3.0 §12/§13。
  - 验证：后端 unit 2828 passed（2 main 预存）+ 前端 566 passed + build 通过；页面 200。